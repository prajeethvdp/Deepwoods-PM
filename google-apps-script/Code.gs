...
function handleSendEmail(payload) {
  try {
    ...
    // Prepare logo blob from LOGO_BASE64_DATA
    var logoBlob = null;
    try {
      if (typeof LOGO_BASE64_DATA !== 'undefined' && LOGO_BASE64_DATA) {
        var logoBytes = Utilities.base64Decode(LOGO_BASE64_DATA);
        logoBlob = Utilities.newBlob(logoBytes, 'image/png', 'logo.png').setName('logo.png');
      }
    } catch (logoErr) {
      Logger.log('Logo blob creation error: ' + logoErr);
    }

    // Ensure htmlBody replaces base64 / external logo src with cid:deepwoodsLogo if needed
    var processedHtmlBody = htmlBody;
    if (logoBlob) {
      // Replace base64 data URIs or raw github logo URLs with cid:deepwoodsLogo so Gmail loads it inline
      processedHtmlBody = processedHtmlBody.replace(/src=["']data:image\/png;base64,[^"']+["']/gi, 'src="cid:deepwoodsLogo"');
      processedHtmlBody = processedHtmlBody.replace(/src=["']https:\/\/raw\.githubusercontent\.com\/[^"']+["']/gi, 'src="cid:deepwoodsLogo"');
    }
